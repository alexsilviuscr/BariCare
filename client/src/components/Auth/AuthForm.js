...
export default function AuthForm(props) {
    ...
    const handleLogin = async (event) =>  {
        ...
        try {
            const response = await axios.post("https://baricare-app-server.up.railway.app/auth/login", { email, password });
    ...
    const handleSignUp = async (event) =>  {
        event.preventDefault();
        try {
            await axios.post("https://baricare-app-server.up.railway.app/auth/register", { username, email, password });
            notifySuccess();
            setTimeout(() => {
                router.push('/login');
            }, 6000);
